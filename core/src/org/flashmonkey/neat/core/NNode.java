...
public class NNode extends Neat {
	...
	public NNode clone() {
		...
		newNode.activation_count = 0;
		newNode.dup = null;
		newNode.analogue = null;
		newNode.is_traversed = false;
		newNode.inner_level = 0;
		newNode.ftype = NeatConstant.SIGMOID;
		
		// cloned
		newNode.type = type;
		newNode.id = id;
		newNode.gen_node_label = gen_node_label;
   ...
   */
	public int depth(int xlevel, Network mynet, int xmax_level) {
		...
		while (itr_link.hasNext()) {
			...
			NNode _ynode = _link.getInputNode();
	...
	public void flushbackOLD() {
		...
		if (type != NeatConstant.SENSOR) {
			...
			while (itr_link.hasNext()) {
				...
				if (_link.getInputNode().activation_count > 0)
