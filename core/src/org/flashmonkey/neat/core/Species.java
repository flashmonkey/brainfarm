...
public class Species extends Neat {
	...
	public void adjust_fitness() {
		...
		for (j = 0; j < size1; j++) {
			...
			if (age <= 10)
				_organism.fitness = _organism.fitness * Neat.age_significance;
		...
		Comparator cmp = new CompareOrganismsByFitness();
		...
		num_parents = (int) Math
				.floor((Neat.survival_thresh * ((double) size1)) + 1.0);
	...
	public double count_offspring(double skim) {
		...
		while (itr_organism.hasNext()) {
			Organism _organism = ((Organism) itr_organism.next());
			//System.out.println("Expected offspring " + expected_offspring);
   ...
   */
	public boolean reproduce(int generation, Population pop,
		...
		for (count = 0; count < expected_offspring; count++) {
			...
			} else if ((RandomUtils.randomDouble() < Neat.mutate_only_prob)
					|| poolsize == 1) {
			...
			// Otherwise we should mate
				...
				if ((RandomUtils.randomDouble() > Neat.mate_only_prob)
						|| (_dad.genome.getId() == mom.genome.getId())
	...
	public void print_to_file(IOseq xFile) {
		...
		while (itr_organism.hasNext()) {
			...
			s2.append(fmt4.format(_organism.genome.getId()));
			s2.append(" Fitness: ");
