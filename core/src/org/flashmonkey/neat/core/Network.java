...
public class Network extends Neat {
	...
	public ArrayList<NNode> getInputs() {
		return inputs;
	}

	public void setInputs(ArrayList<NNode> inputs) {
		this.inputs = inputs;
	}

	public ArrayList<NNode> getOutputs() {
		return outputs;
	}

	public void setOutputs(ArrayList<NNode> outputs) {
		this.outputs = outputs;
	}

	public ArrayList<NNode> getAllNodes() {
		return allnodes;
	}

	public void setAllnodes(ArrayList<NNode> allnodes) {
	 ...
	 */
	public Network(ArrayList<NNode> in, ArrayList<NNode> out, ArrayList<NNode> all, int xnet_id) {
		inputs = in;
		outputs = out;
		allnodes = all;
		name = null;
	...
	public boolean activate() {
		...
		while (outputsoff() || !onetime) {
			...
			for (NNode node : allnodes) {
				//NNode _node = ((NNode) itr_node.next());

				if (node.type != NeatConstant.SENSOR) {

					node.activesum = 0.0; // reset activation value
					node.active_flag = false; // flag node disabled

					itr_link = node.incoming.iterator();
					...
					while (itr_link.hasNext()) {
						...
						if (!_link.isTimeDelayed()) {
							...
							add_amount = _link.getWeight()
									* _link.getInputNode().get_active_out();
							if (_link.getInputNode().active_flag
									|| _link.getInputNode().type == NeatConstant.SENSOR)
								node.active_flag = true;

							node.activesum += add_amount;
						} else {
							add_amount = _link.getWeight()
									* _link.getInputNode().get_active_out_td();
							node.activesum += add_amount;
						}
			...
			//while (itr_node.hasNext()) {
				...
				if (node.type != NeatConstant.SENSOR) {
					...
					if (node.active_flag) {
						...
						if (node.ftype == NeatConstant.SIGMOID)
							node.activation = NeatRoutine.fsigmoid(
									node.activesum, 4.924273, 2.4621365);
						node.activation_count += 1.0;
					}
				}
			}
			onetime = true;
		...
		if (potin_node == potout_node)
			...
			while (itr_link.hasNext()) {
				...
				if (!_link.isRecurrent()) {
					...
					if (!_link.getInputNode().is_traversed) {
						_link.getInputNode().is_traversed = true;
						if (is_recur(_link.getInputNode(), potout_node, level, thresh))
							return true;
					}
				}
			}
			...
			for (NNode node : allnodes) {
				...
				if (node.type != NeatConstant.SENSOR) {
					...
					while (itr_link.hasNext()) {
						Link _link = ((Link) itr_link.next());
						if (!_link.isTimeDelayed()) {
							add_amount = _link.getInputNode().get_active_out();
							if (_link.getInputNode().active_flag
									|| _link.getInputNode().type == NeatConstant.SENSOR) {
								node.active_flag = true;
							}
							...
							add_amount = _link.getInputNode().get_active_out_td();
							node.activesum += add_amount;
						}
					} // End for over incoming links
				} // End if _node.type !=SENSOR
			...
			for (NNode node : allnodes) {
				if (node.type != NeatConstant.SENSOR) {
					...
					if (node.active_flag) {
						node.last_activation2 = node.last_activation;
						node.last_activation = node.activation;
						node.activation = node.activesum;
						node.activation_count += 1.0;
					}
				}
			}
