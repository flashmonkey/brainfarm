...
public class ChartXY extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6868703365917103314L;
	...
	public void drawCode() {
		...
		for (int j = 0; j < azioni.size(); j++) {
			...
			if (_e.tipo == CodeConstant.NODO_N) {
				...
				xs = graph_kx_span + _e.getx() * kx - rx / 2
						+ ((rx - lengthS) / 2);
				; // + nx2*kx;
				...
				if (_e.is_empty)
					g2.drawOval((int) px, (int) py, (int) raggio_base,
							(int) raggio_base);
				else {
					g2.setColor(_e.c);
			...
			// middle chain
			//
			if ((_e.tipo == CodeConstant.LINE_MC)
					|| (_e.tipo == CodeConstant.LINE_H_MC)) {
				// System.out.print("\n -> line_m1 ");
			...
			// 
			// 
			if ((_e.tipo == CodeConstant.LINE_FC)
					|| (_e.tipo == CodeConstant.LINE_H_FC)) {
				// System.out.print("\n -> line_fc ");
				Vertex _p2 = _e.getVertex(1);
				xa = _e.getx(0);
				ya = _e.gety(0);
				...
				modulo = Math.sqrt(Math.pow((xb - xa), 2)
						+ Math.pow((yb - ya), 2));
			...
			if ((_e.tipo == CodeConstant.LINE_SE)
					|| (_e.tipo == CodeConstant.LINE_H_SE)) {
				...
				modulo = Math.sqrt(Math.pow((x2 - x1), 2)
						+ Math.pow((y2 - y1), 2));
				...
				for (int l = 0; l < 3; l++) {
					double xx1 = (Triangle[l][0] * _cos_alfa - Triangle[l][1]
							* _sin_alfa + x2);
					double yy1 = (Triangle[l][0] * _sin_alfa + Triangle[l][1]
							* _cos_alfa + y2);
					Triangle[l][0] = graph_kx_span + xx1 * kx - nx2;
					Triangle[l][1] = (graph_ky_span + graph_axis_y)
							- (yy1 * ky - ny2);
				}
			...
			// draw line with direction
			if ((_e.tipo == CodeConstant.LINE_LC)
					|| (_e.tipo == CodeConstant.LINE_H_LC)) {
				// System.out.print("\n -> line_lc ");
				Vertex _p2 = _e.getVertex(1);
