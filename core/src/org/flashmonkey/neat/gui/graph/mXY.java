package org.flashmonkey.neat.gui.graph;
...
public class mXY {
	 ...
	 * con len > 1 ; se esistono li prolunga in verticale ipotecando i posti
	...
	public void computeMacro(Vector vEdge) {
		...
		for (iA = 0; iA < dim_x; iA++) {
			...
			if (_o != null) {
				...
				// System.out.print("\n piazzo Vertex=" + _o.name +
				// " su colonna x= " + _o.barycentre);
		...
		for (int layer = 0; layer < (dim_y - 1); layer++) {
			...
			while (!fatto) {
				...
				for (iA = 0; iA < dim_x; iA++) {
					...
					// System.out.print("\n   ..ciclo " + tenta +
					// " con indice nodo su layer(" + La + ") = " + iA);
					// view(matrix);
					...
					if (_o != null) {
						...
						if (size_inp > 0) {
							...
							for (iB = 0; iB < dim_x; iB++) {
								...
								if ((_py != null)
										&& (!_py.is_traversed)
										&& (_o.neighbors_inp.contains(_py))
										&& (_o.has_child(_py) || _py
												.has_child(_o))) {
									// System.out.print("\n     trovato possibile successore di "+_o.name+
									// " = "+ _py.name);
									// System.out.print(" ( tipo --> "+_py.type+" )");

									int ll2 = GrafRoutine
											.hamming(_py, vEdge, 0);
									// System.out.print("\n     distanza da nodo reale = "+
									// ll2);
									if (ll2 < minimo) {
										minimo = ll2;
										_py_optimum = _py;
							...
							if (done) {
								...
								for (int k = 0; k < max_range; k++) {
									...
									if ((raggio >= 0) && (raggio < dim_x)) {
										...
										// System.out.print("\n    okay indice "
										// +
										// raggio+" corretto : verifico se matrix["+raggio+"] è libero!");
					...
					if (!_py.is_traversed) {
						...
						for (int k = 0; k < max_range; k++) {
							...
							if ((raggio >= 0) && (raggio < dim_x)) {
								...
								// se raggio è nei limiti allora verifico se il
								// posto è disponibile
								//
								// System.out.print("\n    okay indice " +
								// raggio+" corretto : verifico se matrix["+raggio+"] è libero!");
			...
			// verso il basso
			// In pratica cerca i nodi virtuali su Lb e se il nodo è una
			...
			for (iB = 0; iB < dim_x; iB++) {
				...
				if (_o != null) {
					...
					// if (_o.type == 4)
					{
						// okay nodo virtuale , vedo se fà parte un gruppo di
						// nodi virtuali
						...
						if (l.size() > 0) {
							...
							for (i = 0; i < l.size(); i++) {
								...
								// System.out.print("\n leggo elem(" + i +
								// ") = " + e.name);
								// se il nodo non è stato marcato lo imposta
								// come marcato e
		 ...
		 * first_elem = 9999; for (j = 0; j < dim_y; j++) { //
		 * System.out.print("\n riga .."+j); for (i = 0; i < dim_x; i++) { //
		 * System.out.print("\n     colonna.."+i);
		 * 
		 * if (matrix[j][i] != null) {
		 * 
