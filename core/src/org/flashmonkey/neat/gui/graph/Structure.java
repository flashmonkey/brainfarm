...
public class Structure {
	...
	public void generate_planeXY() {
		...
		while (itr_plane.hasNext()) {
			...
			if (_plane.vEdge.size() > 0) {
				...
				// +" ha "+_plane.dim_y+" righe  e avrà "+xdim+" colonne");
	...
	public void compute_depth_y() {
		...
		/*
		 * 
		 * System.out.print("\n Dopo primo calcolo da out verso input la situazione è : "
	...
	public void LoadGenome(Genome _g) {
		...
		while (itr_gene.hasNext()) {
			...
			int inode_num = _gene.getLink().getInputNode().getId();
			int onode_num = _gene.getLink().getOutputNode().getId();

			weight = _gene.getLink().getWeight();

			boolean recur;
			if (_gene.getLink().isRecurrent())
				recur = true;
			else
				recur = false;
