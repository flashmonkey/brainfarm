...
public class Generation extends JPanel implements ActionListener, ItemListener {
	...
	public void startProcess(INeatContext context) {
		//logger.debug(context.getExperiment().toString());
		startNeat(context);
		/*try {
			logger.debug("Experiment data coming from " + context.getExperiment().getDataSource().toString());

			logger.debug(" generation:      data input  is  "
					+ EnvConstant.DATA_INP);
			logger.debug(" generation:      data output is  "
					+ EnvConstant.DATA_OUT);
			logger.debug(" generation:      fitness class   "
					+ EnvConstant.CLASS_FITNESS);
			// logger.debug(" generation:      unit input are  "+
			// EnvConstant.NR_UNIT_INPUT);
			// logger.debug(" generation:      unit output are "+
			// EnvConstant.NR_UNIT_OUTPUT);

			if ((EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_GENOME)
					&& (!EnvConstant.FORCE_RESTART))
				logger.debug(" generation:      start from genome : "
						+ EnvConstant.NAME_GENOMEA);

			if ((EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_NEW_RANDOM_POPULATION)
					&& (!EnvConstant.FORCE_RESTART)) {
				logger
						.debug(" generation:      start from rnd population : "
								+ EnvConstant.NAME_CURR_POPULATION);
				logger
						.debug(" generation:                       max unit : "
								+ EnvConstant.NR_UNIT_MAX);
				logger
						.debug(" generation:                      recursion : "
								+ EnvConstant.RECURSION);
				logger
						.debug(" generation:                    % Prob link : "
								+ EnvConstant.PROBABILITY_OF_CONNECTION);
				logger
						.debug(" generation:      prefix genome random      : "
								+ EnvConstant.PREFIX_GENOME_RANDOM);
			}

			if (EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_OLD_POPULATION)
				logger
						.debug(" generation:      start from old population : "
								+ EnvConstant.NAME_CURR_POPULATION);

			if (EnvConstant.FORCE_RESTART)
				logger
						.debug(" generation:      start temporary forced from last population : "
								+ EnvConstant.NAME_CURR_POPULATION);

			logger.debug(" generation:      number of epochs       : "
					+ EnvConstant.NUMBER_OF_EPOCH);
			logger.debug(" generation:      prefix of species file : "
					+ EnvConstant.PREFIX_SPECIES_FILE);
			logger.debug(" generation:      prefix of winner  file : "
					+ EnvConstant.PREFIX_WINNER_FILE);

			if (EnvConstant.ACTIVATION_PERIOD == EnvConstant.AUTOMATIC)
				logger
						.debug(" generation:      number of activaction A(t) = f(depth(net)) (automatic)");
			if (EnvConstant.ACTIVATION_PERIOD == EnvConstant.MANUAL)
				logger
						.debug(" generation:      number of activaction A(t) = "
			...
			logger
					.debug(" generation: error during generation.startProcess() :"
							+ e1);
		}
...
*/
	...
	public boolean startNeat(INeatContext context) {
		boolean rc = false;
		...
		//try {
			...
			//fitnessClass = context.getFitnessImpl();
			//logger.info("context: " + context.getNeat() + " " + fitnessClass);
			//context.getNeat().setMaxFitness(fitnessClass.getMaxFitness());

			//if (context.getExperiment().getDataSource() == DataSource.CLASS) {
				...
				EnvConstant.NR_UNIT_INPUT = Integer.parseInt(ObjRet_inp
						.toString());
						*/
				//inputClass = context.getInputImpl();
				//context.getNeat().setNumberOfInputs(inputClass.getNumUnit());
...
/*
				...
				EnvConstant.NUMBER_OF_SAMPLES = Integer.parseInt(ObjRet_inp
						.toString());*/
				//context.getNeat().setNumberOfSamples(inputClass.getNumSamples());
				/*// data output
				//
				...
				EnvConstant.NR_UNIT_OUTPUT = Integer.parseInt(ObjRet_tgt
						.toString());*/
				//outputClass = context.getOutputImpl();
				//context.getNeat().setNumberOfOutputUnits(inputClass.getNumUnit());
			//}

			//if (context.getExperiment().getDataSource() == DataSource.FILE) {
			...
			//logger.debug("size inp -> " + context.getNeat().getNumberOfInputs());
			//logger.debug("size out -> " + context.getNeat().getNumberOfOutputUnits());
			//logger.debug("sample   -> " + context.getNeat().getNumberOfSamples());

		//} catch (Exception ed) {
		//	logger.debug(" generation: error in startNeat() " + ed);
		//}

		try {

			logger.debug(" generation:      read parameter file of neat ...");

			//Neat u_neat = new Neat();
			//u_neat.init();
			/*rc = u_neat.readParametersFromFile(EnvRoutine.getDefaultParameterFileName());

			if (!rc) {
				logger.debug(" generation: error in read "
						+ EnvRoutine.getDefaultParameterFileName());
				return false;
			}*/
			...
			//
			//if (context.getExperiment().getStartFrom() == StartFrom.GENOME && 
			//	!EnvConstant.FORCE_RESTART) {
				...
				if (!rc) {
					logger
							.debug(" generation:   error open "
									+ EnvRoutine.getJneatFileData(EnvConstant.NAME_GENOMEA));
					return false;
				}*/

				/*logger.debug(" generation:      open file genome "
						+ EnvRoutine.getJneatFileData(EnvConstant.NAME_GENOMEA)
						+ "...");
				...
				logger
						.debug(" generation:  ok! created genome id -> "
								+ id);
			...
			if (EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_NEW_RANDOM_POPULATION
					&& (!EnvConstant.FORCE_RESTART)) {
				logger
						.debug(" generation:      cold start from random population.. ");
			...
			if ((EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_OLD_POPULATION)
					|| (EnvConstant.FORCE_RESTART)) {
				/*logger
						.debug(" generation:      warm start from old population -> "
								+ EnvRoutine
...
/*
			for (expcount = 0; expcount < context.getNeat().num_runs; expcount++) {

				logger.debug(" generation:      Spawned population ...");

				if ((EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_GENOME)
						&& (!EnvConstant.FORCE_RESTART))
					u_pop = new Population(u_genome, context.getNeat().pop_size);

				if ((EnvConstant.TYPE_OF_START == EnvConstant.START_FROM_NEW_RANDOM_POPULATION)
						&& (!EnvConstant.FORCE_RESTART))
					u_pop = new Population(context.getNeat().pop_size, (u_inp_unit + 1),
				...
				logger.debug(" generation:      Verifying Spawned Pop....");
				u_pop.verify();
				...
				for (gen = 1; gen <= EnvConstant.NUMBER_OF_EPOCH; gen++) {
					...
					boolean esito = epoch(context.getNeat(), u_pop, gen,
...
*/
			...
			/*u_pop.print_to_file_by_species(EnvRoutine
					.getJneatFileData(EnvConstant.NAME_CURR_POPULATION));*/
			/*logger
					.debug(" generation:      saved curr pop file "
							+ EnvRoutine
									.getJneatFileData(EnvConstant.NAME_CURR_POPULATION));*/
			logger.debug(" generation:  READY for other request");

		}

		catch (Throwable e1) {
			logger.debug(" error in generation.startNeat() " + e1);
		}
			...
			try {
				...
				for (count = 0; count < EnvConstant.NUMBER_OF_SAMPLES; count++) {
					...
					for (int j = 0; j < EnvConstant.NR_UNIT_INPUT; j++) {
						...
						in[j] = inputClass.getInput(input);
					}
					...
					for (int j = 0; j < EnvConstant.NR_UNIT_OUTPUT; j++)
						out[count][j] = ((NNode) _net.getOutputs().get(j))
								.getActivation();
...
*/
		...
		if (success) {
			try {
				...
				if (EnvConstant.TYPE_OF_SIMULATION == EnvConstant.SIMULATION_FROM_CLASS) {
					...
					for (count = 0; count < EnvConstant.NUMBER_OF_SAMPLES; count++) {
						...
						for (int j = 0; j < EnvConstant.NR_UNIT_OUTPUT; j++) {
							...
							tgt[count][j] = outputClass.getTarget(target);
						}
					}
				...
				double[] fitness = fitnessClass.computeFitness(ns, nn, out, tgt);

				fit_dyn = fitness[0];//Array.getDouble(ObjRet_fit, 0);
				err_dyn = fitness[1];//Array.getDouble(ObjRet_fit, 1);
				win_dyn = fitness[2];//Array.getDouble(ObjRet_fit, 2);
	...
	public boolean epoch(Neat _neat, Population pop, int generation,
		...
		try {
			...
			if (((generation % _neat.print_every) == 0) || (win)) {
				...
				logger
						.debug(" generation:      write/rewrite file specie  "
...
*/
			...
			if (!EnvConstant.REPORT_SPECIES_TESTA.equalsIgnoreCase("")) {
				...
				doc2.insertString(doc2.getLength(),
						EnvConstant.REPORT_SPECIES_TESTA, textPane2
								.getStyle(My_styles[1]));
				doc2.insertString(doc2.getLength(),
						EnvConstant.REPORT_SPECIES_CORPO, textPane2
								.getStyle(My_styles[3]));
				doc2.insertString(doc2.getLength(),
						EnvConstant.REPORT_SPECIES_CODA, textPane2
								.getStyle(My_styles[1]));

				if (win)
					logger
							.debug(" generation:    This specie contains the  << WINNER >> ");

				textPane2.setCaretPosition(doc2.getLength());

				if (!(EnvConstant.FIRST_ORGANISM_WINNER == null)) {
					int idx = ((Organism) EnvConstant.FIRST_ORGANISM_WINNER).genome.getId();
			...
			drawCurve(riga1, pop.getHighest_fitness(), generation, pop
					.getSpecies().size(), _neat.pop_size);
